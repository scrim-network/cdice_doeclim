...
void doeclim_DICE_init(DICE *dice)
{
  ...
  doeclim_init(clim->t2co, clim->kappa, clim->dt, clim->ns,
...
void doeclim_hindcast(DICE *dice)
{
  ...
  for(int t=0; t<clim->hind_ns; t++) {
    
    doeclim_ts(t, clim->dt, clim->ns, clim->forc, 
            clim->flnd, clim->powtoheat, clim->cal, clim->cas,
            clim->taucfl, clim->taukls, clim->taucfs, clim->tauksl,
  ...
  // Index (clim-hind_ns - 1) should now contain the initial values
...
void doeclim_dice_ts(DICE *dice, int ts)
{
  ...
  // DICE time step.
  int start = clim->hind_ns + ((ts-1) * config->tstep);
  int finish = clim->hind_ns + (ts * config->tstep);
  ...
  for(int i=start; i<finish; i++) {
    ...
    doeclim_ts(i, clim->dt, clim->ns, clim->forc, 
	       clim->flnd, clim->powtoheat, clim->cal, clim->cas,
	       clim->taucfl, clim->taukls, clim->taucfs, clim->tauksl,
	       clim->bsi, clim->IB, clim->A, clim->fso,
	       clim->taudif, clim->Ker, clim->temp, clim->temp_landair, clim->temp_sst, 
...
void doeclim_load_hind_forc(DICE *dice)
{
  ...
  FILE * fp;
  fp = fopen("./giss_forc_component_1900_2010.txt", "r");

  int counter = 0;
  float temp_forc;
	float ghg, o3, sh2o, refa, aie, bc, snow, stra, solar, land;
  while(!feof(fp)) {
    if (fscanf(fp, "%g %g %g %g %g %g %g %g %g %g", &ghg, &o3, &sh2o, &refa, &aie, &bc, &snow, &stra, &solar, &land) != 10) { break; }
    
    temp_forc = ghg + o3 + sh2o + stra + solar + land + 
    						dice->clim.alpha * (refa + aie + bc + snow);

    dice->clim.forc[counter] = double(temp_forc);    

    counter++;
    if (counter >= dice->clim.hind_ns) {
      break;
    }
  }
  fclose(fp);
...
//
// Applies the calibration for doeclim using the climate sensitivities
// that define the SOW.
//
...
void apply_calibration(DICE *dice)
{
	...
	// Loop through the cs SOW
	for(j=0; j<dice->ctrl.csSamples; j++) {

		this_cs = dice->clim.cs_sample_vec[j];
		
		// Loop through the calibration data
		for(i=0; i<dice->clim.n_calpoints; i++) {
		...
		if(ilow == ihigh) {
			dice->clim.ocean_diff_vec[j] = dice->clim.kv_cal[ilow];
			dice->clim.alpha_vec[j] = dice->clim.alpha_cal[ilow];
		}
		else {
			cs_range = dice->clim.cs_cal[ihigh] - dice->clim.cs_cal[ilow];
			cs_diff = this_cs - dice->clim.cs_cal[ilow];
			dice->clim.ocean_diff_vec[j] = ((cs_diff/cs_range) * (dice->clim.kv_cal[ihigh] - dice->clim.kv_cal[ilow])) + dice->clim.kv_cal[ilow];
			dice->clim.alpha_vec[j] = ((cs_diff/cs_range) * (dice->clim.alpha_cal[ihigh] - dice->clim.alpha_cal[ilow])) + dice->clim.alpha_cal[ilow];
		}
		
		// Reset the diffh and diffl variables for the next cs sample
		diffh = 999.9;
		diffl = -999.9;
