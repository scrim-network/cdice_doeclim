...
void calcExog(DICE *dice)
{
  ...
  econ->partfract[0] = econ->partfract2010;
  ...
  for (int t=1; t<tlim; t++)
    {
      ...
      if (t < 19) {
				carb->forcoth[t] = carb->fex0+(1.0/18.0)*(carb->fex1-carb->fex0)*double(t);
      }
      else {
      ...
      // Fraction of emissions under control regime
      if (t > econ->periodfullpart) {
				econ->partfract[t] = econ->partfractfull;
      }
      else {
				econ->partfract[t] = econ->partfract2010+(econ->partfractfull-econ->partfract2010)*(double(t)/econ->periodfullpart);
      }
  ...
  if(ctrl->use_doeclim == 0) {
    clim->c1 = clim->c10+clim->c1beta*(clim->t2xco2-2.9);
  }
...
void calcEcon1(DICE *dice)
{
  ...
  if(ctrl->use_doeclim == 1) {
    temp = clim->temp[clim->hind_ns-1] - clim->temp[clim->i1900];
    //temp = clim->temp[clim->hind_ns-1];
  }
  else {
    temp = clim->tatm[0];
  }
  ...
  // Carbon price
  econ->cprice[0] = econ->pbacktime[0]*pow((dvars->miu[0]/econ->partfract[0]),(econ->expcost2-1));
  ...
  // Abatement costs
  econ->abatecost[0] = econ->ygross[0]*econ->cost1[0]*pow(dvars->miu[0],econ->expcost2)*pow(econ->partfract[0],(1-econ->expcost2));
...
void calcEcon(DICE *dice, int t)
{
  ...
  // Where do we get the temperature from?
  if(ctrl->use_doeclim == 1) {
    int this_index = clim->hind_ns+(t*config->tstep)-1;
    temp = clim->temp[this_index] - clim->temp[clim->i1900];
  ...
  // Carbon price
  econ->cprice[t] = econ->pbacktime[t]*pow((dvars->miu[t]/econ->partfract[t]),(econ->expcost2-1));
  ...
  // Abatement costs
  econ->abatecost[t] = econ->ygross[t]*econ->cost1[t]*pow(dvars->miu[t],econ->expcost2)*pow(econ->partfract[t],(1-econ->expcost2));
